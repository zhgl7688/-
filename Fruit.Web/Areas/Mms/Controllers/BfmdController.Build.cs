...
namespace Fruit.Web.Areas.Mms.Controllers
{
    public partial class BfmdController : Controller
    {
        ...
        public ActionResult Edit(string id)
        {
            ...
            {
                TypeCode = db.Database.SqlQuery<ComboItem>("select Text Text, Value Value from sys_code where " + string.Format("{0}", "CodeType='bfmd'")).ToList();
            }
    ...
    public partial class BfmdApiController : ApiController
    {
        ...
        public object Get()
        {
            ...
            using (var db = new LUOLAI1401Context())
            {
                return pageReq.ToPageList<base_customFieldListModel>(db.Database, "a.id ,b.Text TypeCode_RefText ,a.TypeCode ,a.TypeName ", "base_customField a LEFT JOIN [Bcp.Sys].dbo.sys_code b ON a.TypeCode = b.Value ", sbCondition.ToString(), "id", "desc");
            }
        }
        public object Post(JObject post)
        {
            var form = post["form"].ToObject<base_customField>(JsonExtension.FixJsonSerializer);
            using (var db = new LUOLAI1401Context())
            {
