...
namespace Fruit.Web
{
    ...
    public class HttpControllerSelector : DefaultHttpControllerSelector
    {
        ...
        public IEnumerable<string> GetControllerFullName(HttpRequestMessage request, string controllerName)
        {
            ...
            var selector = from n in namespaces
            ...
            if (selector.Count() == 0)
            {
                // 支持 ApiController 后缀的处理
                selector = from n in namespaces
                           join k in keys on string.Format("{0}.{1}Api{2}", n, controllerName,
                           DefaultHttpControllerSelector.ControllerSuffix).ToLower() equals k.ToLower()
                           select k;
            }
            return selector;
